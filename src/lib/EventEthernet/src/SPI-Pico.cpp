...
arduino::MbedSPI::MbedSPI(int miso, int mosi, int sck) : _miso(digitalPinToPinName(miso)), _mosi(digitalPinToPinName(mosi)), _sck(digitalPinToPinName(sck)), spiSemaphore(1)
{
}

arduino::MbedSPI::MbedSPI(PinName miso, PinName mosi, PinName sck) : _miso(miso), _mosi(mosi), _sck(sck), spiSemaphore(1)
{
}
