...
public:
  static uint32_t pinAddrMask;
  static uint32_t pinDataMask;
  static uint8_t chip;
  ...
  {
    // LOG_TRACE("addr=(hex)", DebugLogBase::HEX, addr, ", data=(hex)", data);
    write8bit(addr, (uint8_t)((data >> 8) & 0xFF));
    write8bit(addr + 1, (uint8_t)(data & 0xFF));
    return 1;
  }

  static inline uint32_t mapValueToBit(uint32_t value, uint8_t pin)
  {
    return ((value >> pin) & 0x01);
  }
  static inline uint32_t mapBitToValue(uint32_t value, uint8_t pin)
  {
    return ((value & 0x01) << pin);
  }
  static uint8_t write8bit(uint16_t addr, uint8_t data)
  {
    uint32_t value = mapBitToValue(data >> 0, Pin_D0) |
                     mapBitToValue(data >> 1, Pin_D1) |
                     mapBitToValue(data >> 2, Pin_D2) |
                     mapBitToValue(data >> 3, Pin_D3) |
                     mapBitToValue(data >> 4, Pin_D4) |
                     mapBitToValue(data >> 5, Pin_D5) |
                     mapBitToValue(data >> 6, Pin_D6) |
                     mapBitToValue(data >> 7, Pin_D7);
    // LOG_TRACE("addr=(hex)", DebugLogBase::HEX, addr, ", data=(hex)", data, ", value=(hex)", value);

    uint32_t addrBit = addr2Bit(addr);
    gpio_put_masked(pinAddrMask, addrBit);
    gpio_set_dir_out_masked(pinDataMask);
    gpio_put_masked(pinDataMask, value);
  ...
  static inline uint32_t addr2Bit(uint32_t addr)
  {
    return addr; // A0-A9 is mapped to GP0-GP9 => no need to do bit mapping
  }

  static inline uint8_t read8bit(uint16_t addr)
  {
    uint32_t addrBit = addr2Bit(addr);
    gpio_set_dir_in_masked(pinDataMask);
    // LOG_TRACE("addr=(hex)", DebugLogBase::HEX, addr, ", addrBit=(hex)", DebugLogBase::BIN, addrBit);
    ...
    asm volatile("nop");
    asm volatile("nop");
    uint32_t value = gpio_get_all() & pinDataMask;
    // LOG_TRACE("gpio_get_all()=(hex)", DebugLogBase::HEX, value);

    // delay to fullfill tCS/tRD = 65ns (min)
    asm volatile("nop");
    asm volatile("nop");
    asm volatile("nop");
    asm volatile("nop");
    ...
    //  map pin values to data bits
    uint8_t data = (mapValueToBit(value, Pin_D0) << 0) |
                   (mapValueToBit(value, Pin_D1) << 1) |
                   (mapValueToBit(value, Pin_D2) << 2) |
                   (mapValueToBit(value, Pin_D3) << 3) |
                   (mapValueToBit(value, Pin_D4) << 4) |
                   (mapValueToBit(value, Pin_D5) << 5) |
                   (mapValueToBit(value, Pin_D6) << 6) |
                   (mapValueToBit(value, Pin_D7) << 7);
    // LOG_TRACE("addr=(hex)", DebugLogBase::HEX, addr, ", addrBit=(hex)", addrBit, ", data=(hex)", DebugLogBase::HEX, data);
    return data;
  }
  ...
  {
    uint16_t ret = ((uint16_t)read8bit(addr) << 8) | ((uint16_t)read8bit(addr + 1));
    return ret;
  }
  ...
  uint32_t getSnRX_RSR(uint8_t sn);
...
#define getSn_DIPR(sn, dipr)                                  \
  {                                                           \
    uint16_t value = WIZCHIP_READ(Sn_DIPR(sn));               \
    (dipr)[0] = (uint8_t)(value >> 8);                        \
    (dipr)[1] = (uint8_t)value;                               \
    value = WIZCHIP_READ(WIZCHIP_OFFSET_INC(Sn_DIPR(sn), 2)); \
    (dipr)[2] = (uint8_t)(value >> 8);                        \
    (dipr)[3] = (uint8_t)value;                               \
  }
  // #define getSn_DIPR(sn, dipr) { \
...
#define getSn_TX_WRSR(sn)                                                   \
  {                                                                         \
    uint32_t value = (((uint32_t)WIZCHIP_READ(Sn_TX_WRSR(sn))) << 16);      \
    value += (uint32_t)WIZCHIP_READ(WIZCHIP_OFFSET_INC(Sn_TX_WRSR(sn), 1)); \
    value &= 0x0000FFFF;                                                    \
  }
...
public:
  ...
  uint8_t getChip(void) { return (getIDR() >> 8); }
  ...
  static void writeIR(uint16_t ir) { setIR(ir); }
  static uint16_t readIR() { return getIR(); }
  static void writeIMR(uint16_t imr) { setIMR(imr); }
  static uint16_t readIMR() { return getIMR(); }
